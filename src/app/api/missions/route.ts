...
export async function POST(request: NextRequest) {
  try {
    const { title, icon, type, deadline, start_date, end_date, recurring_days, attachments, target_child_id, coin_reward, xp_reward } = await request.json();
    ...
    const { data, error } = await getSupabaseServer().from('missions').insert({
      ...
      target_child_id: target_child_id || null,
